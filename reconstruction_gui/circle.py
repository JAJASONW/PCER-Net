...
def find_arc(points_input, points_fit, p2p_dis, r_ball_multiple, n_gap):
    r_ball = p2p_dis * r_ball_multiple
    circle_string = ''
    ...
    for r_index in range(row):
        [_, idx, _] = pcd_tree.search_radius_vector_3d(points_fit[r_index], r_ball)
        # x = np.asarray(pcd.points)[idx[1:]]
        neighborindex_list = idx[0:]
        # print(neighborindex_list)
        if neighborindex_list:
            circle_string = circle_string + '1'
        else:
            circle_string = circle_string + '0'
    # print(circle_string)
    t_result = get_str_range(circle_string, n_gap)
    return t_result
...
def get_r_ball(points_input):
    ...
    [_, _, dis] = pcd_tree.search_knn_vector_3d(pcd.points[10], 10)
    p2p_dis = dis[1:][0]
    return p2p_dis
...
if __name__=='__main__':
    ...
    for fn in fns:
        ...
        t = find_arc(cloud, circle, p2p_dis, r_ball_multiple=5000, n_gap=36)
        t_rad = deg2rad(t)
        fitcircle = sample_circle(t_rad, circle_center, radius, normal, u)
        np.savetxt(save_dir + fn + "_fit.txt", fitcircle, fmt="%.5f")
